...
import styles from "../styles/contentUpdate.module.css";
...
const contentUpdate = ({ cookies }) => {
  ...
  const onClick = (e) => {
    ...
    console.log(Data.get('contentId'));

    axios.patch("https://greeneachdomain.tk/content", Data, {
      headers: { token: cookies.get("userInfo") },
    })
      .then((res) => {
        if (res.status === 200) {
          alert("컨텐츠가 업데이트 되었습니다.");
...
export default withCookies(contentUpdate);
